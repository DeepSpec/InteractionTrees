...
Lemma notau_untaus (t : itree E R) : notau t -> untaus t t.
Proof. split; [ auto | constructor ]. Qed.

Lemma untaus_untaus' (t t' : itree E R) :
...
Lemma untaus_tau (t1 t2 : itree E R) :
...
Lemma untaus'_finite_taus (t t' : itree E R) :
...
Lemma untaus_finite_taus (t t' : itree E R) :
    untaus t t' -> (finite_taus t <-> finite_taus t').
Proof.
  intros []. symmetry. apply untaus'_finite_taus; assumption.
Qed.
...
Lemma eutt_untaus_1 (t1 t2 : itree E R) :
...
Lemma untaus_equiv (t t' : itree E R) : untaus t t' -> t ~ t'.
Proof.
  intros H.
  pfold. split.
...
Lemma untaus_bind {E R S} (t : itree E R) (k : R -> itree E S) tk' :
  untaus (t >>= k) tk' ->
  (exists t', untaus t t' /\ untaus (t' >>= k) tk').
Proof.
  intros [Hnotau Huntaus].
  remember (t >>= k)%itree as tk eqn:Etk.
  generalize dependent t.
  induction Huntaus; intros t' Etk'; subst.
  - rewrite match_bind in Etk'.
    ...
    + exists (Ret r). split.
      ...
      * rewrite match_bind; simpl; rewrite <- Etk'.
        rewrite untaus_tau.
        split; auto.
    + edestruct IHHuntaus as [t2 [Ht2 Htk2]]; eauto.
      eexists; split; eauto.
      rewrite untaus_tau; auto.
  - exists t'. split.
    + eapply notau_untaus, notau_bind; eauto.
    + apply notau_untaus; auto.
Qed.
