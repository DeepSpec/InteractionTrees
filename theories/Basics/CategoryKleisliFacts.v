...
Section BasicFacts.
  ...
  Context {Mr : MRet m}.
  Context {Mb : MBind m}.
  Context {Eq1P : @Eq1Equivalence m Eq1}.
  Context {ML : @MonadLawsE m Eq1 Mr Mb}.
  ...
  Lemma pure_id {A: Type}: (id_ (C := Kleisli m) A ⩯ @pure _ _ A A (id_ A))%cat.
  Proof.
    reflexivity.
  Qed.

  Fact compose_pure {A B C} (ab : A -> B) (bc : B -> C) :
    (pure (m := m) ab >>> pure bc ⩯ pure (ab >>> bc))%cat.
  Proof.
    intros a.
  ...
  Lemma pure_inl {A B}
    : pure (m := m) (b := A + B) inl_ ⩯ inl_.
  Proof. reflexivity. Qed.

  Lemma pure_inr {A B}
    : pure (m := m) (b := A + B) inr_ ⩯ inr_.
  Proof. reflexivity. Qed.

  Lemma case_pure {A B C} (ac : A -> C) (bc : B -> C) :
    case_ (pure ac) (pure bc)
      ⩯ pure (m := m) (@case_ _ Fun _ _ _ _ _ ac bc).
  Proof.
    intros []; reflexivity.
  Qed.
  ...
  Lemma unit_l_pure (A : Type) :
    unit_l ⩯ @pure m _ (void + A) A unit_l.
  Proof.
    intros [[]|]. reflexivity.
  Qed.

  Lemma unit_l'_pure (A : Type) :
    unit_l' ⩯ @pure m _ A (void + A) unit_l'.
  Proof.
    reflexivity.
  Qed.

  Lemma unit_r_pure (A : Type) :
    unit_r ⩯ @pure m _ (A + void) A unit_r.
  Proof.
    intros [|[]]. reflexivity.
  Qed.

  Lemma unit_r'_pure (A : Type) :
    unit_r' ⩯ @pure m _ A (A + void) unit_r'.
  Proof.
    reflexivity.
  Qed.
  ...
  Fact bimap_id_pure {A B C} (f : B -> C) :
    bimap (id_ A) (pure f) ⩯ pure (m := m) (bimap (id_ A) f).
  Proof.
  ...
  Fact bimap_pure_id {A B C} (f : A -> B) :
    bimap (pure f) (id_ C) ⩯ pure (m := m) (bimap f id).
  Proof.
    unfold bimap, Bimap_Coproduct.
  ...
  Lemma map_inl_case_kleisli:
    forall (a1 b1 b2 c1 c2 : Type) (f1 : Kleisli m a1 b1) (g1 : Kleisli m b1 c1) (g2 : Kleisli m b2 c2),
      map (m := m) inl f1 >>> case_ (map inl g1) (map inr g2) ⩯ map inl (f1 >>> g1).
  Proof.
