...
Instance eq_itree_interp_state {E F S R} (h : E ~> Monads.stateT S (itree F)) :
  Proper (@eq_itree E R ==> @eq S ==>
          @eq_itree F (S * R)) (interp_state h _).
Proof.
  repeat intro. pupto2_init. revert_until R.
...
Proof.
  ...
  apply itree_eq_tau.
  assert (pointwise_relation _ eq_itree (fun x : R => interp f R (Ret x)) (fun x => Ret x)).
...
Definition eh_eq {A B : Type -> Type} f g := forall X, pointwise_relation (A X) (@eutt B X) (f X) (g X).
...
Proof.
  ...
  assert (pointwise_relation _ eq_itree (fun x : X => interp f X (Ret x)) (fun x => Ret x)).
