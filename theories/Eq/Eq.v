...
Global Instance eqit_bind_ {E R S} b1 b2 k :
  Proper (going (eqit eq b1 b2) ==>
          eqit eq b1 b2) (@ITree._bind E R S k (fun t => @bind (itree E) _ R S t k)).
Proof.
...
Lemma unfold_map {E R S} (f : R -> S) (t : itree E R) :
...
Proof.
  ...
  apply eqit_bind.
  - reflexivity.
  - intros ?. rewrite unfold_map. reflexivity.
Qed.

Lemma map_ret {E A B} (f : A -> B) (a : A) :
    @ITree.map E _ _ f (Ret a) ≅ Ret (f a).
Proof.
  intros. rewrite unfold_map, bind_ret_l; reflexivity.
Qed.
...
Proof.
  ...
  - intros. subst.
    ...
    + cbn in *.
      ...
      * setoid_rewrite (itree_eta ma).
        rewrite Hobma, bind_ret_l.
        reflexivity.
      * exists a; split; auto.
        rewrite (itree_eta ma), Hobma in H1.
...
Proof.
  ...
  - intros. inv Heqtr.
    ...
    + cbn in *.
      ...
      * right.
        destruct a.
        setoid_rewrite (itree_eta ma) in H1.
