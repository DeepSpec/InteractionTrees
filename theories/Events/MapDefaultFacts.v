...
Section MapFacts.
  ...
  Global Instance eq_map_refl {d} : Reflexive (eq_map d).
  Proof.
    red. intros. unfold eq_map. tauto.
  Qed.    

  Global Instance eq_map_sym {d} : Symmetric (eq_map d).
  Proof.
    repeat intro.
    unfold eq_map in H.
    rewrite H.
  ...
  Global Instance eq_map_trans {d} : Transitive (eq_map d).
  Proof.
    repeat intro. 
    unfold eq_map in *.
  ...
  Definition map_default_eq d {E} 
    ...
    fun t1 t2 => forall s1 s2, eq_map d s1 s2 -> eutt (prod_rel (eq_map d) RR) (t1 s1) (t2 s2).

  End Relations.

  Lemma eq_map_add:
    forall (d : V) (s1 s2 : map) (k : K) (v : V), eq_map d s1 s2 -> eq_map d (add k v s1) (add k v s2).
  Proof.
  ...
  Lemma eq_map_remove:
    forall (d : V) (s1 s2 : map) (k : K), eq_map d s1 s2 -> eq_map d (remove k s1) (remove k s2).
  Proof.
    intros d s1 s2 k H.
  Admitted.
  ...
  Lemma handle_map_eq : 
    forall d E X (s1 s2 : map) (m : mapE d X),
      eq_map d s1 s2 ->
      eq_itree (prod_rel (eq_map d) eq) (handle_map m s1) ((handle_map m s2) : itree E (map * X)).
  Proof.
    intros.
  ...
  Lemma run_map_id d {E X} (t : itree (mapE d +' E) X) :
