...
Proof.
  unfold interp_state, interp, iter, Iter_stateTM, iter, MonadIter_itree, internalize; cbn.
...
Lemma eutt_interp_state_iter {E F S A A' B B'}
  ...
  (forall a a' s1 s2, RS s1 s2 -> RA a a' ->
     eutt (fun a b => RS (fst a) (fst b) /\ RB (snd a) (snd b))
          (interp_state h (CategoryOps.iter (C := ktree _) t1 a) s1)
          (interp_state h (CategoryOps.iter (C := ktree _) t2 a') s2)).
Proof.
...
Lemma interp_state_iter {E F } S (f : E ~> stateT S (itree F)) {I A}
  ...
  : forall i, state_eq (State.interp_state f (ITree.iter t i))
                  (iter t' i).
Proof.
  unfold iter, Iter_stateTM, iter, MonadIter_itree in *; cbn.
...
Lemma interp_state_iter' {E F } S (f : E ~> stateT S (itree F)) {I A}
      (t  : I -> itree E (I + A))
  : forall i, state_eq (State.interp_state f (ITree.iter t i))
                       (iter (fun i => State.interp_state f (t i)) i).
Proof.
