...
Section par.
  ...
  Definition _par
             ...
             (t1 : itreeF E unit (itree E unit))
             (t2 : itreeF E unit (itree E unit)) : itree E unit :=
    match t1, t2 with
    | RetF _, _ => go t2
    | _, RetF _ => go t1
    | TauF t'1, TauF t'2 =>
  ...
  CoFixpoint par' (t1 t2 : itree E unit) : itree E unit :=
    _par par' (observe t1) (observe t2).
...
Section Denote.
  ...
  Fixpoint denote_imp (s : stmt) : itree eff unit :=
    ...
    | Skip => ret tt
    | Par a b => par (denote_imp a) (denote_imp b)
    end.
...
Definition interp_imp1  {E A} (t : itree (ImpState +' E) A) : stateT env (itree E) A :=
...
Definition eval_imp (s: stmt) : stream -> env -> itree void1 (stream * (env * unit)) :=
  fun strm env =>
  interp_state handle_Or (interp_imp1 (denote_imp s) env) strm.

CoFixpoint zeros : stream := cons true zeros.
CoFixpoint ones : stream := cons false zeros.

Definition example2 : stmt :=
  "x" ::= 0 ;;;
  "y" ::= 0 ;;;
