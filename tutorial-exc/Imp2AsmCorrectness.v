...
Definition relabel_output  : fin (1 + 1) -> fin (1 + (1 + 1)) :=
  bimap (id_ _) inr_.

Lemma while_asm_exc_correct (e : list instr) (p : asm 1 (1 + 1)) : 
...
Qed.
  ...
  Definition link_with_exc : ktree_fin (impExcE +' Reg +' Memory +' IOE) (1 + 1) 1  :=
    (case_ (C := ktree_fin _ )
           (id_ _)
            (fun _ : fin 1 => v <- trigger (Throw _ tt);; match v : void with end )).
  ...
  Proof.
    ...
    cbn in *. rewrite seq_asm_correct. rewrite relabel_asm_correct.
    (* remember (app_asm (id_asm) (raise_asm) ) as app. *)
    assert (denote_asm (app_asm (@id_asm 1) raise_asm) ⩯ bimap (denote_asm id_asm) (denote_asm raise_asm) ).
    { rewrite app_asm_correct. reflexivity. }
    (* rewrite H. *)
    unfold id_asm.
    assert (denote_asm (app_asm (@pure_asm 1 1 id) raise_asm) ⩯ bimap (denote_asm (pure_asm id)) (denote_asm raise_asm) ).
    { rewrite app_asm_correct. reflexivity. }
    rewrite H0. cbn. rewrite pure_asm_correct. cbn.
    ...
    - cbn. rewrite bind_bind. setoid_rewrite bind_ret_l. cbn.
      Transparent denote_asm. tau_steps.  apply eqit_Vis. intros [].
  Qed.

  Lemma early_link_with_exc (p1 p2: ktree_fin (impExcE +' Reg +' Memory +' IOE) 1 (1 + 1) ) :
...
Section Correctness.
  ...
  Lemma compile_output_correct : forall e,
      bisimilar eq
        ((v <- denote_expr e ;; trigger (Print v)) : itree (impExcE +' stateE +' IOE) unit)
        ((denote_list (compile_output e)) : itree (impExcE +' Reg +' Memory +' IOE) unit).
  Proof.
  ...
  Definition label_rel (r : unit + unit) (l : fin 2) :=
             match r with
             | inl tt => proj1_sig l = 0
             | inr _ => proj1_sig l = 1 end.
  ...
  Proof.
    ...
    - (* Assign *)
     ...
     assert (forall r, throw_prefix (Err := unit) (E := (stateE +' IOE)) (trigger (Put x r) ) ≈ trigger (Put x r);; Ret (inl tt)  ).
     {
    ...
    - (* Output *) 
      ...
      rewrite throw_prefix_bind. rewrite throw_prefix_denote_expr.
      rewrite bind_bind. setoid_rewrite bind_ret_l.
      assert (forall r, throw_prefix (Err := unit) (E := (stateE +' IOE)) (trigger (Print r) ) ≈ 
                                trigger (Print r);; Ret (inl tt)  ).
     { 
       intros. rewrite bind_trigger. setoid_rewrite throw_prefix_ev. 
       apply eqit_Vis. setoid_rewrite tau_eutt. intros []. rewrite throw_prefix_ret. reflexivity.
     }
...
(** Correctness of the compiler.
  ...
  Proof.
    ...
    cbn. setoid_rewrite bind_ret_l. cbn.
    (* setoid_rewrite app_asm_correct.  rewrite <- fold_to_itree'. cbn. *)

    assert (denote_asm (app_asm (@id_asm 1) raise_asm) ⩯ bimap (denote_asm id_asm) (denote_asm raise_asm) ).
    { rewrite app_asm_correct. reflexivity. }
    unfold id_asm.
    assert (denote_asm (app_asm (@pure_asm 1 1 id) raise_asm) ⩯ bimap (denote_asm (pure_asm id)) (denote_asm raise_asm) ).
    { rewrite app_asm_correct. reflexivity. } clear H.
    do 5 red in H0. setoid_rewrite H0. cbn. unfold unsubm.
    unfold to_bif, ToBifunctor_ktree_fin. setoid_rewrite bind_ret_l.
    unfold case_, Case_Kleisli, case_sum.
    unfold id_, Id_sub, id_, Id_Fun. cbn.
    ...
    - assert (l = fS f0). { red in Hl. apply unique_fin. auto. } subst.
      ...
      cbn. rewrite bind_bind. rewrite bind_trigger. unfold resum, ReSum_id, id_, Id_IFun.
