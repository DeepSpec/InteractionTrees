...
Section Fin.
  ...
  Definition FinC (n m: nat) := Fin n -> Fin m.

  Definition to_nat' {n: nat} (k: Fin n): nat := proj1_sig (to_nat k).

  Global Instance Eq_FinC : Eq2 FinC :=
    fun n m f g => forall (k: Fin n), to_nat' (f k) = to_nat' (g k).

  Global Instance Id_FinC : Id_ FinC :=
    fun n k => k.

  Global Instance Cat_FinC : Cat FinC :=
    fun n m p (f:(Fin n) -> (Fin m)) (g: Fin m -> Fin p) => cat f g: Fin n -> Fin p. 
  ...
  Global Instance Case_FinC : CoprodCase FinC plus :=
    fun {n m p} (k : FinC n p) (k' : FinC m p) (x : Fin (n + m)) =>
